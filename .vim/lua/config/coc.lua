...
function M.setup_keymappings()
  ...
  api.nvim_set_keymap('i', '<Tab>', 'pumvisible() ? "<C-n>" : "v:lua.check_back_space() ? <TAB>"', opts_silent_expr)
  ...
  api.nvim_set_keymap('n', leader .. 'f', '<Plug>(coc-format-selected)', opts_silent)
  api.nvim_set_keymap('x', leader .. 'f', '<Plug>(coc-format-selected)', opts_silent)
end
